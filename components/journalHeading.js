...
export default async function JournalTopics() {
    ...
    return (
        <p className="text-2xl md:text-4xl my-2 lg:text-5xl text-gray-700">
            {
                posts?.reverse().map((item, index) => {
                    if (index <  5 ) {
                        return (
                            <Link className={"no-underline my-2 text-gray-600 mr-2"} href={`./journal/${encodeURIComponent(item._id)}`} key={item._id}>
